...
allowed_origins = [
    ...
    for origin in os.getenv(
        "ALLOWED_ORIGINS",
        "https://artinsurance.io,https://www.artinsurance.io,https://artinsurance.github.io",
    ).split(",")
    if origin.strip()
]
app.add_middleware(
...
def run_equasis_lookup(imo: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="equasis-lookup-", dir=DATA_DIR) as temporary:
        ...
        for item in payload.get("screenshots", []):
            ...
            image_path = (candidate if candidate.is_absolute() else ROOT / candidate).resolve()
            if image_value and image_path.is_relative_to(output.resolve()) and image_path.is_file():
        ...
        return {
            ...
            "equasisStatus": result.get("status", "failed"),
            "warning": result.get("error", "") if result.get("status") != "captured" else "",
        }
