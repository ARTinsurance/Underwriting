...
def site_path(path: Path) -> str:
    ...
    try:
        ...
        # API jobs deliberately write into an isolated runtime directory outside
        # the source tree.  Preserve that absolute path for the API process; it
        # validates the path against the job output directory before reading it.
        return str(resolved)
